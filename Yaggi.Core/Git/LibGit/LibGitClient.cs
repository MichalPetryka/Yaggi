				...
				if (authenticationProvider != null)
				{
					GitCallbacks.CredentialAcquireCallback acquireCredentials = (data, ptr, fromUrl, types, _) =>
					{
						...
						switch (types)
						{
							...
							case GitCredentialType.Username:
								(bool, string[]) input3 = authenticationProvider(prompt, new[] { ("Username", usernameFromUrl, false) });
								if (!input3.Item1)
									return GitErrorCode.User;
							...
							case GitCredentialType.SshMemory:
							...
							// TODO: implement
							case GitCredentialType.SshCustom:
							//return GitNative.CredentialSshCustomNew(data);
							case GitCredentialType.SshInteractive:
							//return GitNative.CredentialSshInteractiveNew(data);
